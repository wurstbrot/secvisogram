...
class InputFieldHeaderLine:
  ...
  @property
  def is_input_field_header(self):
...
def parse_line(line: str) -> Union[HeaderLine, InputFieldHeaderLine, ImageLine, TextLinkLine, str]:
  """
  parses a given line from a markdown file into an object with more information. Could be one of `HeaderLine`,
   `InputFieldHeaderLine`, `ImageLine` oder `TextLinkLine`. In case no such line is found, the initial string is returned
  ...
  if bold_search:
    return InputFieldHeaderLine(text=bold_search.group(2))
...
def get_dir_content(dir_path: Path, language: str = "en", files_only=False) -> List[str]:
  ...
  # Identify content of subdirectories
  for d in sorted(dirs):
    for f in sorted(files):
      if f.startswith(d):
        sub_dir_content.append(f)
    sub_dir_content.append(d)

  # Then add the remaining files. We want to have the content of the current object before the content of subdirs
  for f in sorted(files):
    if f not in sub_dir_content:
      result.append(f)

  # Now add the subdir content as well
  for c in sub_dir_content:
    result.append(c)

  # if "acknowledgment" in str(dir_path):
  #   print("------------")
  #   print(f"DIRS: {dir_path}")
...
def main(args):
  ...
  with open(output_file_path, "w+", encoding="UTF-8") as output_file:
    ...
    for file_path, (depth, content) in _STATE[PATH_TO_DEPTH_AND_CONTENT].items():
      ...
      elif isinstance(first_line, InputFieldHeaderLine):
